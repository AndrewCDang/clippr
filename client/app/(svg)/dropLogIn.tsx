...
export default function DropLogIn({height,width, fill}:Svg) {
    ...
    return (
    <svg className={` group-hover:fill-primary-f  ${height ? `h-${height}` : 'h-10'} ${width ? `w-${width}` : 'w-10'} ${fill ? `fill-${fill}` : null }   cursor-pointer transition duration-200 group-hover:scale-90 ease-in-out`} data-name="Layer 1" viewBox="-8 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
