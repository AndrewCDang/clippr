...
async function barberPage({ params }: any) {
  ...
  const latestReviews = writtenReviews?.slice(0, 6).sort((a, b) => {
    return b.id - a.id;
  });
  ...
  return (
    <main className=" my-8 rounded-xl shadow-lg flex-1 flex flex-col justify-between p-4 relative">
      <img
        alt=""
        src={barber?.bg_banner}
        className={`absolute h-40 w-full rounded-t-xl z-0  top-0 left-0 bg-light object-cover opacity-[80%]`}
      ></img>
      <section className="z-0">
        <div className="w-40 h-40 rounded-full overflow-hidden custom-shadow z-10">
          <img
            className="w-full aspect-square object-cover"
            src={`${barber?.UserTable?.profilePicture}`}
        ...
        <div className="pb-2 flex flex-wrap justify-between items-start mt-2">
          <div>
            ...
            <div className="flex gap-1">
              <h2 className="text-secondary">{`${barber?.barber_level
                .charAt(0)
                .toUpperCase()}${barber?.barber_level.slice(1)} barber`}</h2>
            </div>
            <div className="flex items-center">
              {barberStars}
              <h3 className="ml-1 font-medium">
                {barber?.reviews_stars && barber?.reviews_stars.toFixed(2)}
      ...
      <section className="grid sm:grid-cols-1 md:grid-cols-12">
        <section className="md:col-span-8 flex flex-col ">
          ...
          <section className="my-8">
                  ...
                  <section key={i} className=" flex gap-2">
                    <div>
                      <div className="w-12 h-12 rounded-full overflow-hidden bg-light ">
                        {review.UserTable.profilePicture ? (
                          <img
                            className="w-full h-full object-cover"
                            src={`${review.UserTable.profilePicture}`}
                    ...
                    <div className="flex flex-col">
                      ...
                      <div className="flex items-center">
                        ...
                        <h4 className="text-xs text-light">{`(${review.created_at.slice(
                          8,
                          10
                        )}/${review.created_at.slice(
                          5,
                          7
                        )}/${review.created_at.slice(0, 4)})`}</h4>
                      </div>
            ...
            {latestReviews && latestReviews.length > 4 && (
              <a className="underline cursor-pointer w-fit mx-auto">See More</a>
            )}
            {latestReviews?.length == 0 && (
              <a className="text-light w-fit mx-auto">No current reviews</a>
            )}
          </section>
        </section>
