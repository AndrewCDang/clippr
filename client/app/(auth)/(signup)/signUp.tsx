"use client"
import {AuthFormSignUp} from '@/app/(auth)/AuthSignUp'
...
export function SignUp(){
    ...
    return(
        <section className={`${registerIsOpen ? 'fixed' : 'hidden' } flex bg-opacity-50 top-0 left-0 z-50 w-full bg-secondary h-full justify-center`}>
            <section className='z-10 relative place-self-center bg-white p-4 flex flex-col h-fit gap-2 rounded-xl max-w-[400px] w-[80%] min-w-[320px]'>
                ...
                <AuthFormSignUp/>
                <h5>Already have an account? <strong className='cursor-pointer' onClick={()=>logInInstead()}>Log-in</strong> here</h5>
                <a onClick={()=>registerClose()} className='absolute top-0 right-0 m-2'>
                    <XSvg height={8} width={8} fill={'primary'} strokeWidth={12}/>
                </a>
            </section>
            <a onClick={()=>registerClose()} className='absolute w-full h-full z-0'></a>
        </section>
    )
}
