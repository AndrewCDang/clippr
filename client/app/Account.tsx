...
const Account = () => {
    ...
    const getUser = async() => {
        ...
        const { data:picUrl, error: profileError} = await supabase.from('UserTable')
            .select('profilePicture')
            .eq('email',data.session?.user.email)
            .single()
        if(picUrl){
            setProfilePicture(String(picUrl.profilePicture))
        }
    }
    ...
    return(
        <section className="relative flex justify-end items-center">
            {userName    && <h3 className="mr-2">Hello, <a className="text-medium">{userName}</a></h3>}
            <button ref={accountDropRef} onClick={()=>toggleAccountModal()} >
                {
                    ...
                    :
                    <svg className={`${accountIsOpen ? 'stroke-primary' : null} stroke-primary nav-svg-profile cursor-pointer`} width="48px" height="48px" viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg">
            ...
            {!isUserLoggedIn ?
            <section className={`${accountIsOpen ? 'flex' : 'hidden'} mt-2 absolute top-[100%] flex-col border-[1px] border-light rounded-xl overflow-hidden [&>*:first-child]:border-0 hover:drop-shadow-xl transition duration-200 ease-in-out`}>
            ...
            :
            <section className={`${accountIsOpen ? 'flex' : 'hidden'} mt-2 absolute top-[100%] flex-col border-[1px] border-light rounded-xl overflow-hidden [&>*:first-child]:border-0 hover:drop-shadow-xl transition duration-200 ease-in-out`}>
                < AccountDropdown toggleFunctions={handleAppointments}  dropList="View Appointments" dropSvg={< DropLogIn fill={"light"}/>}  />
