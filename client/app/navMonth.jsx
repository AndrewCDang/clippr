...
export  default function NavMonth({navHeightToggle, setBookingDate, bookingDate}){
    ...
    const setMonthCal = (days, firstGap, month) => {
        ...
        const dateHandler = (monthIndex, day, monthItem) => {
        ...
        const updateBookingState = useCallback((day,monthItem)=>{
                    ...
                    <div style={{width:'fit-content'}}  className='mx-auto'>
                            <div className="flex flex-row nav-cal-row w-min">
                            {
                                [...Array(7)].map((day, dayIndex)=>{
                                    return <div key={dayIndex} style={{cursor:'none'}} className='w-8 h-8 border-primary '><a>{daysArray[dayIndex]}</a></div>
                                })
                            }
                            </div>
                                            ...
                                            {[...Array(6)].map((_, weekIndex) => (
                                                <div key={weekIndex} className="flex flex-row nav-cal-row w-min">
                                                    {monthItem.calendar.slice(weekIndex * 7, (weekIndex + 1) * 7).map((day, dayIndex) => (
                                                        <div onClick={()=>day !=='' ? dateHandler(monthIndex,day,monthItem) : null}
                                                            key={dayIndex}
