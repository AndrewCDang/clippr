                ...
                <section ref={interactRef} className={`absolute top-[50%] left-[50%] translate-x-[-50%] ${opacityOn ? 'translate-y-[-50%] ':'translate-y-[0%]'} transition-transform duration-300 ease-in-out`}>
                    <div className={`bg-white inset-0 m-auto p-4 rounded-xl relative shadow-xl`}>
                        <div className="flex items-center gap-4 pb-2 mb-2 border-b-[0.5px] border-light">
                            <div className="w-32">
                                <img className=' w-full  aspect-square object-cover rounded-lg' src={(appointment.UserTable as UserItem).profilePicture}></img>
                            </div>
                            <div className="flex h-fit justify-between w-fit gap-8">
                                <div className="flex flex-col gap-4">
                                    <div className="flex flex-col">
                                        <h1 className=''>{(appointment.UserTable as UserItem).first_name} {(appointment.UserTable as UserItem).last_name}</h1>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <section className='flex flex-col my-2'>
                            ...
                            <div className="grid grid-cols-3">
                                ...
                                <h3 className='p-2 font-semibold col-span-2'>{daysWeek[appointmentDate.getDay()]} - {appointmentDate.getDate()}/{appointmentDate.getMonth()+1}/{appointmentDate.getFullYear()}</h3>
                            </div>
                            <div className="grid grid-cols-3">
                                <h3 className="p-2">time</h3>
                            {appointment.cut_time.length === 2 ?
                                <h3 className={`p-2 font-semibold col-span-2`}>{`${appointment.cut_time[0].split('-')[0]}-${appointment.cut_time[1].split('-')[1]} `}</h3>
                                :
                                <h3 className={`p-2 font-semibold col-span-2`}>{appointment.cut_time[0]}</h3>
                            }
                            </div>
                            <div className="grid grid-cols-3">
                                <h3 className="p-2">cut</h3>
                                <h3 className='p-2 font-semibold col-span-2'>{appointment.cut_name}</h3>
                            </div>
                            <div className="grid grid-cols-3">
                                <h3 className="p-2">price</h3>
                                <h3 className='p-2 font-semibold grid-cols-3'>£{appointment.cut_price}</h3>
                            </div>
                        </section>
                        <div className="flex gap-2">
                            <Button  svg={<div className="scale-x-[-1]"><ChatSVG/></div>} variant={1} text={'Message'}/>
                            <div ref={cancelRef}>
